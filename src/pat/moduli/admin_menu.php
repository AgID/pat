...
<div class="leftmenu">        
	<ul class="nav nav-tabs nav-stacked">
		<li class="nav-header">Amministrare <? echo $configurazione['denominazione_etrasparenza']; ?></li>
		
		<? // VERIFICO QUALI FUNZIONI ELABORARE NEL MENU 
		foreach ($funzioniMenu as $menuTemp) { 
			if ($menuTemp['nomeMenu'] != '') {
				// verifico se questa � la sezione attiva
				$classeAct = '';
				...
				if (is_array($menuTemp['sottoMenu']) AND count($menuTemp['sottoMenu'])) {
					...
					foreach ($menuTemp['sottoMenu'] as $sottoSezTemp) { 
						...
						echo "<li class=\"".$classeActSec."\"><a href=\"admin_pat.php?menu=".$menuTemp['menu']."&amp;menusec=".$sottoSezTemp['menuSec']."\">".$sottoSezTemp['nomeMenu']."</a></li>";
					}
					echo "</ul></li>";
				} else {
					// pulsante diretto
					echo "<li class=\"".$classeAct."\"><a href=\"admin_pat.php?menu=".$menuTemp['menu']."\"><span class=\"".$menuTemp['iconaPiccola']."\"></span> ".$menuTemp['nomeMenu']."</a></li>";
				}
			}
		}
		?>
