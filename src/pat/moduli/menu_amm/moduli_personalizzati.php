...
require ('./pat/classi/admin_moduli_personalizzati.php');
...
switch ($azione) {
	...
	case "lista" :
		...
		if ($datiUser['permessi'] != 10) {
			...
			include ('./pat/admin_template/moduli_personalizzati/tab_start.tmp');
			$visualizzaInterfaccia = true;
			foreach ($listaTabella as $istanzaOggetto) {
				include ('./pat/admin_template/moduli_personalizzati/tab_row.tmp');
			}
			
			include ('./pat/admin_template/moduli_personalizzati/tab_end.tmp');
		}
	break;
	...
	case "aggiungi" :
		...
		if ($datiUser['permessi'] != 10) {
			...
			include ('./pat/admin_template/moduli_personalizzati/form.tmp');
	...
	case "modifica" :
		...
		if ($datiUser['permessi'] != 10 AND !$aclTrasparenza['admin']) {
			...
			if ($aclTrasparenza['admin'] AND $datiUser['permessi'] == 10) {
