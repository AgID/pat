...
if ($interfaccia=="semplice") {
	if (!$valore) {
		...
		///////// province
		echo "<select ".$classeStr." id=\"".$nome."\" name=\"".$nome."\"><option value=\"\">qualunque provincia</option>";
		...
		///////// comuni
		echo "<select ".$classeStr." id=\"".$nome."\" name=\"".$nome."\"><option value=\"\">qualunque</option>";
	...
	if (!$valore) {
		...
		if ($valore==1) {
				...
				function DropDownList_".$nome."regioni_onchange(v) {
				    ...
				    for(var i in province".$nome.") {
				        if(province".$nome."[i][2] == key) {
				            lista_push($(\"".$nome."\"), province".$nome."[i][1], province".$nome."[i][0]);
				        }
				    }
			...
			echo "</select>"; 
			echo "<select ".$classeStr." id=\"".$nome."\" name=\"".$nome."\" style=\"display:none;\"><option value=\"\">qualunque provincia</option>";
			if ($valoreVero != '') {
				foreach($arrayJsProvince as $provincia) {
					$stringa = '';
			...
			echo "<script type=\"text/javascript\">
				...
				function DropDownList_".$nome."regioni_onchange(v) {
				    ...
				    var key = v.value;
				    //cancello tutti le province
				    $(\"".$nome."_LISTAPROVINCE\").options.length = 1;
				    //cancello tutti i comuni
				    $(\"".$nome."\").options.length = 1;
				    //creo lista provincia
				    for(var i in province".$nome.") {
				        if(province".$nome."[i][2] == key) {
				            lista_push($(\"".$nome."_LISTAPROVINCE\"), province".$nome."[i][1], province".$nome."[i][0]);
				        }
				    }
				    $(\"".$nome."_LISTAPROVINCE\").style.display = 'inline';
				}
				function DropDownList_".$nome."province_onchange(v) {
				    ...
				    for(var i in comuni".$nome.") {
				        if(comuni".$nome."[i][2] == key) {
				            lista_push($(\"".$nome."\"), comuni".$nome."[i][1], comuni".$nome."[i][0]);
				        }
				    }
				    $(\"".$nome."\").style.display = 'inline';
				}
			...
			echo "</select>"; 
			// select province
			echo "<select ".$classeStr." id=\"".$nome."_LISTAPROVINCE\" name=\"".$nome."_LISTAPROVINCE\" onchange=\"return DropDownList_".$nome."province_onchange(this)\" style=\"display:none;margin:0px 4px 0px 0px;\"><option value=\"\">qualunque</option>";
			if ($valoreVero != '') {
				foreach($arrayJsProvince as $provincia) {
					if ($provincia[2] == $valoreRegione) {
						$stringa = '';
			...
			echo "</select>";
			//select comuni
			echo "<select ".$classeStr." id=\"".$nome."\" name=\"".$nome."\" style=\"display:none;\"><option value=\"\">qualunque</option>";
			if ($valoreVero != '') {
				foreach($arrayJsComuni as $comuni) {
