...
function cancellaUtente" . $nome . "(valore,nome) {
	...
	if (selectSezioni.length==0) {
		var nuovaOpzione = document.createElement('option');
		nuovaOpzione.text = 'Nessun utente selezionato';
  		nuovaOpzione.value = '';  
  		selectSezioni.add(nuovaOpzione, null); 	
	}
}
...
if ($valoreVero != '' and $valoreVero != 0) {
	...
	if (count($arrayScelte) == 0 OR $valoreVero == '') {
		echo "<option value=\"\">Nessun utente selezionato</option>";
	} else {
		for ($i = 0, $tot = count($arrayScelte); $i < $tot; $i++) {
	...
	echo "<select " . $classeStr . " id=\"" . $nome . "nomi\" name=\"" . $nome . "nomi\" size=\"6\"><option value=\"\">Nessun utente selezionato</option></select>";
...
echo "
	...
	<img src=\"grafica/admin_skin/classic/user_add.gif\" alt=\"Aggiungi un utente alla selezione\" />Aggiungi</a>
        <a class=\"bottoneClassico\" title=\"Togli un utente dalla selezione\" href=\"javascript:cancellaUtente" . $nome . "();\">
	<img src=\"grafica/admin_skin/classic/user_canc.gif\" alt=\"Togli un utente dalla selezione\" />Elimina</a>
";
?>
