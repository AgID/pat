...
function cancellaGruppo" . $nome . "(valore,nome) {
	...
	if (selectSezioni.length==0) {
		var nuovaOpzione = document.createElement('option');
		nuovaOpzione.text = 'Nessun gruppo selezionato';
  		nuovaOpzione.value = '';  
  		selectSezioni.add(nuovaOpzione, null); 	
	}
}
...
if ($valoreVero != '' and $valoreVero != 0) {
	...
	if (count($arrayScelte) == 0 OR $valoreVero == '') {
		echo "<option value=\"\">Nessun gruppo selezionato</option>";
	} else {
		for ($i = 0, $tot = count($arrayScelte); $i < $tot; $i++) {
	...
	echo "<select " . $classeStr . " id=\"" . $nome . "nomi\" name=\"" . $nome . "nomi\" size=\"6\"><option value=\"\">Nessun gruppo selezionato</option></select>";
...
echo "
	...
	<img src=\"grafica/admin_skin/classic/gruppo_piu.gif\" alt=\"Aggiungi un gruppo alla selezione\" />Aggiungi</a>
        <a class=\"bottoneClassico\" title=\"Togli un gruppo dalla selezione\" href=\"javascript:cancellaGruppo" . $nome . "();\">
	<img src=\"grafica/admin_skin/classic/gruppo_meno.gif\" alt=\"Togli un gruppo dalla selezione\" />Elimina</a>
";
?>
