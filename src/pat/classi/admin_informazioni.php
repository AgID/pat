...
class infoAdmin {
	...
	function caricaOggetto($id,$strutturaCompleta = 'no') {
		foreach($this->oggetti as $oggetto) {
			if ($oggetto['id'] == $id) {
				...
				//TODO NICO: OK! se l'oggetto � webservice non fare il parsing struttura classico ma chiamare il web services.
	...
	// funzione di caricamento propriet� oggetto
	...
	function aggiungiImport($arrayValori) {
		...
		// se � un default, resetto i precedenti
		...
		// installo l'IMPORT
		$sql = "INSERT INTO ".$dati_db['prefisso']."oggetti_import (id_utente,id_oggetto,id_categoria,id_ente,num_suc,num_err,reset_import,dati_default,data_importazione,file,settings,id_inseriti,report) VALUES 
				(".$arrayValori['utente'].",".$arrayValori['oggetto'].",".$arrayValori['categoria'].",".$arrayValori['id_ente'].",".$arrayValori['num_suc'].",".$arrayValori['num_err'].",".$arrayValori['reset_dati'].",".$arrayValori['default'].",".time().",\"".addslashes($arrayValori['file'])."\",\"".addslashes(stripslashes(stripslashes($arrayValori['settings'])))."\",\"".$arrayValori['id_inseriti']."\",\"".addslashes(stripslashes(stripslashes($arrayValori['report'])))."\")";
		if( !($result = $database->connessioneConReturn($sql)) ) {
