...
require ('./app/classi/admin_ruoli.php');
...
switch ($azione) {
	...
	case "lista" :
		...
		if (!$aclTrasparenza['ruoli']) {
			...
			include ('./app/admin_template/ruoli/tab_start.tmp');
			$visualizzaInterfaccia = true;
			foreach ($listaTabella as $istanzaOggetto) {
				include ('./app/admin_template/ruoli/tab_row.tmp');
			}
			
			include ('./app/admin_template/ruoli/tab_end.tmp');
	...
	case "aggiungi" :
		...
		if (!$aclTrasparenza['ruoli']) {
			...
			include ('./app/admin_template/ruoli/form.tmp');
		}
	...
	case "modifica" :
		...
		if (!$aclTrasparenza['ruoli']) {
			...
			if (($aclTrasparenza['ruoli'] == 2 OR $istanzaOggetto['id_ente'] == $datiUser['id_ente_admin']) AND ($id != 0 OR $datiUser['permessi']==10 OR $datiUser['permessi']==3)) {
	...
	case "visualizza":
		// carico il ruolo da visualizzare
		$istanzaOggetto = $ruoliClasse->caricaRuolo($id);
		...
		if ($aclTrasparenza['ruoli'] == 1 AND $istanzaOggetto['id_ente'] == 0) {
			...
			motoreLogTrasp('permessonegato', 'Non hai i permessi necessari per visualizzare questo profilo ACL.');
		}
