...
require ('./app/classi/admin_enti.php');
$entiClasse = new enti();
...
switch ($azione) {
		...
		if ($datiUser['permessi'] != 10 and $datiUser['permessi'] != 3) {
			...
			if (isset ($_POST['rispostaForm']) and $azioneSecondaria == 'aggiungi') {
				...
				if ($_FILES['file_logo_semplice']['name'] != '') {			
					...
					if (!(copy($_FILES['file_logo_semplice']['tmp_name'], $uploadPath."/enti_trasparenza/".$idPerAllegati.$prog."O__O".$nomeFileReplace))) {
				...
				if ($_FILES['file_logo_etrasp']['name'] != '') {			
					...
					if (!(copy($_FILES['file_logo_etrasp']['tmp_name'], $uploadPath."/enti_trasparenza/".$idPerAllegati.$prog."O__O".$nomeFileReplace))) {
						// ERRORI NELL'OPERAZIONE
						$operazione = false;
				...
				if ($_FILES['file_organigramma']['name'] != '') {			
					...
					if (!(copy($_FILES['file_organigramma']['tmp_name'], $uploadPath."/enti_trasparenza/".$idPerAllegati.$prog."O__O".$nomeFileReplace))) {
						// ERRORI NELL'OPERAZIONE
						$operazione = false;
				...
				if(!getEntiAttivabili()) {
					$operazione = false;
					$operazioneTesto = "Non puoi attivare ulteriori enti. Limite massimo raggiunto.";
			...
			include ('./app/admin_template/enti/tab_start.tmp');
			$visualizzaInterfaccia = true;
			foreach ($listaTabella as $istanzaOggetto) {
				include ('./app/admin_template/enti/tab_row.tmp');
			}
			
			include ('./app/admin_template/enti/tab_end.tmp');
	...
	case "aggiungi" :
		...
		if ($datiUser['permessi'] != 10 and $datiUser['permessi'] != 3) {
			...
			if(getEntiAttivabili()) {
				...
				include ('./app/admin_template/enti/form.tmp');
			} else {
				motoreLogTrasp('permessonegato', 'Non puoi attivare ulteriori enti. Limite massimo raggiunto.');
			}
	...
	case "modifica" :
		...
		if ($datiUser['permessi'] != 10 AND $datiUser['permessi'] != 3 AND !$aclTrasparenza['admin']) {
			...
			if ($aclTrasparenza['admin'] AND ($datiUser['permessi'] == 10 OR $datiUser['permessi'] == 3 OR $datiUser['id_ente_admin']==$istanzaOggetto['id']) ) {
