...
switch ($azione) {
	...
	case "lista" :
		...
		if (!$aclTrasparenza[$menuSecondario]['lettura']) {
			...
			if (isset ($_POST['rispostaForm']) and $azioneSecondaria == 'aggiungi') {
				...
				$idPagineGeneriche = array();
				for($i = 0; $i < count($_POST['id_pagine_generiche']); $i++) {
					if($_POST['id_pagine_generiche'][$i]) {
						$idPagineGeneriche[] = $_POST['id_pagine_generiche'][$i];
					}
				}
				$_POST['id_pagine_generiche'] = implode(',', $idPagineGeneriche);
				
				if(!count($_POST['id_utenti_finali'])) {
					$_POST['id_utenti_finali'] = '';
				}
	...
	case "modifica" :
		...
		if ($datiUser['permessi'] != 10 AND $datiUser['permessi'] != 3 AND !$aclTrasparenza['admin']) {
			...
			if (($aclTrasparenza[$menuSecondario]['modifica'] OR $istanzaOggetto['id_ente'] == $datiUser['id_ente_admin']) AND ($id != 0 OR $datiUser['permessi']==10 OR $datiUser['permessi']==3)) {
