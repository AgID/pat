...
function selectRicercaStruttura($campo, $etichetta = 'Struttura', $classCampo = 'campoOggetto71', $classForm = 'stileForm75') {
	...
	if ( !($result = $database->connessioneConReturn($sql)) ) {
		mostraAvviso(0,'Errore in questo campo: se presente, � probabile ci sia un errore nella condizione aggiuntiva.');
	}
...
function formazionePersonale($istanzaOggetto) {
	if($istanzaOggetto['formazione_personale'] != '') {
		...
		if(count($array)> 0) {
			$out = '<h4 class="campoOggetto86">Dati sulla formazione</h4><div class="oggetto76"><div class="table-responsive"><table class="table table-bordered table-hover vistaTabella"><tr><th>Descrizione attivit�</th><th>Dal</th><th>Al</th></tr>';
			foreach((array)$array as $t) {
		...
		foreach((array)$listaPerCategoria as $cat => $lista) {
			...
			foreach((array)$lista as $istanzaOggetto) {
			    ...
			    if((!moduloAttivo('privacy') OR ($istanzaOggetto['permessi_lettura']!='H' AND moduloAttivo('privacy')))) {
					...
					if(file_exists($uploadPath . "oggetto_allegati/" . $temp. $istanzaOggetto['file_allegato'])) { // Retrocompatibilit�
...
function selectRicercaTipologiaOgg11($campo, $etichetta = 'Tipologia', $classCampo = 'campoOggetto71', $classForm = 'stileForm75') {
	...
	if($configurazione['visualizza_select_ricerca_scelta_contr_bando']) {
	    ...
	    $istanze[] = array( 'val' => '35', 'eti' => '35-PARTERNARIATO PER L�INNOVAZIONE' );
