...
if(file_exists('codicepers/template/'.$entePubblicato['nome_breve_ente'].'/oggetti/personale/lettura.php')) {
	...
	if ($istanzaOggetto['foto'] != '' and $istanzaOggetto['foto'] != 'nessuno') {
		...
		if ($estFile == 'gif' or $estFile == 'jpg' or $estFile == 'jpeg' or $estFile == 'png' or $estFile == 'bmp') {
			...
			echo "<div class=\"fotoReferente\"><img alt=\"" . $istanzaOggetto['referente'] . "\" src=\"" . $base_url . "moduli/output_media.php?file=" . $documento->tabella . "/" . $istanzaOggetto['foto'] . "&amp;qualita=75&amp;larghezza=120px\" /></div>";
		}
	}
	...
	if($istanzaOggetto['delega'] == 1 and $istanzaOggetto['testo_delega'] != '') {
		echo '<div><h4 class="campoOggetto86"> Consigliere con delega a: </div><div>'.$istanzaOggetto['testo_delega'].'</h4></div>';
	}
	
	echo '<div class="reset"></div>';
	
	echo '<h4 class="campoOggetto86">Contatti</h4>';
	
	if($istanzaOggetto['email_non_disponibile'] == 1 and $istanzaOggetto['email_non_disponibile_text'] != '') {
		echo '<div class="campoOggetto77">Email: '.$istanzaOggetto['email_non_disponibile_text'].'</div>';
	...
	if($istanzaOggetto['email_cert'] != '') {
		echo '<div class="campoOggetto77">Email certificata: <a href="mailto:'.$istanzaOggetto['email_cert'].'">'.$istanzaOggetto['email_cert'].'</a></div>';
	}
	...
	if (count($docRiferiti)) {
		...
		*/
		$outputScreen .= '</ul>';
		echo '<h4 class="campoOggetto86"> Referente per le strutture </h4>'.$outputScreen;
		unset ($outputScreen);
	}
	unset ($docRiferiti);
	unset ($docRif);
	...
	if (count($docRiferiti)) {
		...
		$outputScreen .= '</ul>';
		echo '<h4 class="campoOggetto86"> Procedimenti seguiti come responsabile di procedimento </h4>'.$outputScreen;
		unset ($outputScreen);
	}
	unset ($docRiferiti);
	unset ($docRif);
	...
	if (count($docRiferiti)) {
		...
		$outputScreen .= '</ul>';
		echo '<h4 class="campoOggetto86"> Procedimenti seguiti come responsabile di provvedimento </h4>'.$outputScreen;
		unset ($outputScreen);
	}
	unset ($docRiferiti);
	unset ($docRif);
	...
	foreach((array)$docRiferiti as $oggTmp) {
		$strAncora = $base_url . "index.php?id_oggetto=43&amp;id_cat=" . $oggTmp['id_sezione'] . "&amp;id_doc=" . $oggTmp['id'];
		$valoreLabel = $oggTmp[$docRif->campo_default];
		switch($oggTmp['tipologia']) {
			case 'commissione':
			case 'gruppo consiliare':
				$valoreLabel .= ' (Presidente o capogruppo)';
			break;
			case 'udp':
				$valoreLabel .= ' (Presidente)';
			break;
		}
		$arrayMembri[] = "<li><a href=\"" . $strAncora . "\">" . $valoreLabel . "</a></li>";
	}
	
	$docRiferiti = $docRif->caricaDocumentiCampo('vicepresidente', $istanzaOggetto['id']);
	foreach((array)$docRiferiti as $oggTmp) {
		$strAncora = $base_url . "index.php?id_oggetto=43&amp;id_cat=" . $oggTmp['id_sezione'] . "&amp;id_doc=" . $oggTmp['id'];
		$valoreLabel = $oggTmp[$docRif->campo_default];
		switch($oggTmp['tipologia']) {
			...
			case 'gruppo consiliare':
			case 'udp':
				$valoreLabel .= ' (Vicepresidente)';
			break;
			case 'ci':
			case 'gect':
				$valoreLabel .= ' (Componente effettivo)';
			break;
		}
		$arrayMembri[] = "<li><a href=\"" . $strAncora . "\">" . $valoreLabel . "</a></li>";
	}
	
	$docRiferiti = $docRif->caricaDocumentiCampo('segretari', $istanzaOggetto['id']);
	foreach((array)$docRiferiti as $oggTmp) {
		$strAncora = $base_url . "index.php?id_oggetto=43&amp;id_cat=" . $oggTmp['id_sezione'] . "&amp;id_doc=" . $oggTmp['id'];
		$valoreLabel = $oggTmp[$docRif->campo_default];
		switch($oggTmp['tipologia']) {
			case 'commissione':
			case 'gruppo consiliare':
				$valoreLabel .= ' (Segretario)';
			break;
			case 'udp':
				$valoreLabel .= ' (Segretario questore)';
			break;
		}
		$arrayMembri[] = "<li><a href=\"" . $strAncora . "\">" . $valoreLabel . "</a></li>";
	}
	
	$docRiferiti = $docRif->caricaDocumentiCampo('membro', $istanzaOggetto['id']);
	foreach((array)$docRiferiti as $oggTmp) {
		$strAncora = $base_url . "index.php?id_oggetto=43&amp;id_cat=" . $oggTmp['id_sezione'] . "&amp;id_doc=" . $oggTmp['id'];
		$valoreLabel = $oggTmp[$docRif->campo_default];
		switch($oggTmp['tipologia']) {
	...
	foreach((array)$docRiferiti as $oggTmp) {
		...
		switch($oggTmp['tipologia']) {
			...
			case 'ci':
			case 'gect':
				$valoreLabel .= ' (Componente supplente)';
			break;
		}
		$arrayMembri[] = "<li><a href=\"" . $strAncora . "\">" . $valoreLabel . "</a></li>";
	}
	
	if (count($arrayMembri)) {
		$outputScreen = '<ul>';
		foreach ($arrayMembri as $membro) {
			$outputScreen .= $membro;
		}
		$outputScreen .= '</ul>';
		echo '<h4 class="campoOggetto86"> Membro di </h4>'.$outputScreen;
		unset ($outputScreen);
	}
	unset ($docRiferiti);
	unset ($docRif);
	...
	if (count($docRiferiti)) {
		...
		$outputScreen .= '</ul>';
		echo '<h4 class="campoOggetto86"> Presidente o capogruppo per </h4>'.$outputScreen;
		unset ($outputScreen);
	}
	unset ($docRiferiti);
	unset ($docRif);
	...
	*/
	...
	/* inizio altri dati */
	$altriDati = array();
	if($istanzaOggetto['organo'] == '' and $istanzaOggetto['ruolo_politico'] == '') {
	...
	if($istanzaOggetto['curriculum'] != '' and (!moduloAttivo('privacy') OR (!$istanzaOggetto['omissis'] AND moduloAttivo('privacy')))) {
		...
		$altriDati[] = '<div class="campoOggetto48">Curriculum: <a href="'.$base_url.'moduli/downloadFile.php?file='.$documento->tabella.'/'.urlencode($istanzaOggetto['curriculum']).'">'.$valoreLabel.'</a> ('.round($grandezza/1000).' kb) <img style="vertical-align:middle" src="'.$base_url.'grafica/file/small/'.$estFile.'.gif" alt="File con estensione '.$estFile.'" /></div>';
	}
	...
	if($istanzaOggetto['note'] != '') {
		echo '<div class=""><h4 class="campoOggetto86">Documentazione Art. 14  e Art. 47, c. 1, Dlgs n. 33/2013; Art. 1,2,3,4 l. n. 441/1982</h4><div class="">'.$istanzaOggetto['note'].'</div></div>';
	}
	...
	if($istanzaOggetto['atto_conferimento'] != '' and (!moduloAttivo('privacy') OR (!$istanzaOggetto['omissis'] AND moduloAttivo('privacy')))) {
		...
		echo '<div class="campoOggetto48">Atto di conferimento: <a href="'.$base_url.'moduli/downloadFile.php?file='.$documento->tabella.'/'.urlencode($istanzaOggetto['atto_conferimento']).'">'.$valoreLabel.'</a> ('.round($grandezza/1000).' kb) <img style="vertical-align:middle" src="'.$base_url.'grafica/file/small/'.$estFile.'.gif" alt="File con estensione '.$estFile.'" /></div>';
	}
	
	echo '<div class="reset"></div>';
	
	if($istanzaOggetto['estremi_atto_conferimento'] != '') {
		echo '<div class=""><h4 class="campoOggetto86">Estremi atto di nomina o proclamazione</h4><div class="">'.$istanzaOggetto['estremi_atto_conferimento'].'</div></div>';
	}
	
	if($istanzaOggetto['compensi'] != '') {
		echo '<div class=""><h4 class="campoOggetto86">Compensi connessi alla carica</h4><div class="">'.$istanzaOggetto['compensi'].'</div></div>';
	}
	
	if($istanzaOggetto['importi_viaggi'] != '') {
		echo '<div class=""><h4 class="campoOggetto86">Importi di viaggi di servizio e missioni</h4><div class="">'.$istanzaOggetto['importi_viaggi'].'</div></div>';
	}
	
	if($istanzaOggetto['altri_incarichi'] != '') {
		echo '<div class=""><h4 class="campoOggetto86">Altri incarichi con oneri a carico della finanza pubblica e relativi compensi</h4><div class="">'.$istanzaOggetto['altri_incarichi'].'</div></div>';
	}
	...
	if($istanzaOggetto['altre_info'] != '') {
		echo '<div class=""><h4 class="campoOggetto86">Altre informazioni</h4><div class="">'.$istanzaOggetto['altre_info'].'</div></div>';
	}
	
	formazionePersonale($istanzaOggetto);
	
	if($istanzaOggetto['__archiviata_descrizione'] != '') {
		echo '<div class=""><h4 class="campoOggetto86">Informazioni</h4><div class="">'.$istanzaOggetto['__archiviata_descrizione'].'</div></div>';
	}
	
	if($istanzaOggetto['archivio_informazioni'] != '') {
		echo '<div class=""><h4 class="campoOggetto86">Archivio informazioni</h4><div class="">'.$istanzaOggetto['archivio_informazioni'].'</div></div>';
	}
