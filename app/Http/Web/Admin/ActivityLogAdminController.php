...
/**
 *
 * Controller Acitvity Log
 *
 */
class ActivityLogAdminController extends BaseAuthController
{
    ...
    public function index()
    {
        ...
        $this->breadcrumb->push('Log delle attività', '/');

        $data = [];
        $data['breadcrumbs'] = $this->breadcrumb->show();

        //Dati header della sezione
        $data['titleSection'] = 'Log delle attività';
        $data['subTitleSection'] = 'ATTIVITÀ EFFETTUATE SUI CONTENUTI DELLA TRASPARENZA';
        $data['sectionIcon'] = '<i class="far fa-chart-bar fa-3x"></i>';

        $data['formAction'] = '/admin/activity-log';
        $data['formSettings'] = [
            'name' => 'form_activity-log',
            'id' => 'form_activity-log',
            'class' => 'form_activity-log',
        ];

        render('activity_log/index', $data, 'admin');
    }
    ...
    public function asyncPaginateDatatable()
    {
        ...
        if (Input::isAjax() === true && $validator['is_success'] === true) {
            ...
            $columnName = !empty($columnNameArr[$columnIndex]['data']) ? (int)$columnNameArr[$columnIndex]['data'] : 'activity_log.created_at';
            ...
            $order = setOrderDatatable($columnName, $orderable, 'activity_log.created_at');
            ...
            if (!empty($records)) {
                ...
                foreach ($records as $record) {
                    ...
                    $setTempData = [];
                    $setTempData[] = createdByCheckDeleted(@$record['user']['name'], @$record['user']['deleted']);
...
//                    $setTempData[] = !empty($record['referer']) ? escapeXss($record['referer']) : 'N.D.';
    ...
    /**
     * @throws Exception
     * @url /admin/activity-log/create.html
     * @method GET
     */
    public function create()
    {
        ...
        $data['breadcrumbs'] = $this->breadcrumb->show();
        $data['titleSection'] = 'Log delle attività';
        render('activity-log/activity-log', $data, 'admin');
    }

    /**
     * @throws Exception
     * @url /admin/activity-log/edit.html
     * @method GET
     */
