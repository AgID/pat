...
/**
 * Controller Open data per le News e Notizie
 */
class OpenDataNewsNoticeFrontController extends BaseFrontController
{
    ...
    /**
     * Metodo download open data per le news e notizie,
     * nella sezione Altri contenuti -> Avvisi e News
     * ID sezione 180
     *
    ...
    public function index(): void
    {
        ...
        $queryContents = NewsNoticesModel::where('start_date', '<=', $tomorrow)
    ...
    private function generateCsv($resultContests = null, $resultSection = null): void
    {
        ...
        $fileName = 'News ed avvisi - ' . $resultSection['name'] . '.csv';
        header('Set-Cookie: fileDownload=true; path=/');
        ...
        foreach ($resultContests as $r) {
            ...
            $data = [
                ...
                //Data Notizia
                !empty($r['news_date']) ? convertDateForCsv($r['news_date']) : null,
                //Contenuto
                !empty($r['content']) ? S::chartsEntityDecode(S::stripTags($r['content'])) : null,
                //Url origine dato
