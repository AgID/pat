...
/**
 * Controller Open data per gli Atti di Programmazione
 */
class OpenDataProgrammingActFrontController extends BaseFrontController
{
    ...
    /**
     * Metodo download open data per gli Atti di Programmazione,
     * nella sezione Bandi di gara e contratti -> Atti relativi alla programmazione di lavori, opere, servizi e forniture
     * ID sezione 113
     *
    ...
    public function index(): void
    {
        ...
        $queryContents = ProgrammingActsModel::orderBy('date', 'DESC')
            ->orderBy('object', 'ASC')
            ->skip(Input::get('skip'))
            ->take(Input::get('take'))
            ->get();
    ...
    /**
     * Metodo download open data per gli Atti di Programmazione,
     * nella sezione con ID sezione 297 e 298
     *
     * @url download/open-data/page_id
     * @return void
     * @throws Exception
     */
    public function programming(): void
    {
        // Recupero le informazioni della pagina corrente
        $querySection = SectionsFoModel::select(['id', 'name'])
            ->where('id', uri()->segment(3, 0))
        ...
        $actType = $querySection->id == 297 ? 1 : 2;

        // Recupero i bandi di concorso attivi
        $queryContents = ProgrammingActsModel::where('act_type', $actType)
            ->orderBy('date', 'DESC')
            ->orderBy('object', 'ASC')
            ->skip(Input::get('skip'))
            ->take(Input::get('take'))
    ...
    /**
     * Metodo download open data per gli Atti di Programmazione,
     * nella sezione Opere Pubbliche -> Atti di programmazione delle opere pubbliche
     * ID sezione 167
     *
     * @url download/open-data/page_id
     * @return void
    ...
    public function indexActsPublicWorks(): void
    {
        ...
        // Recupero i bandi di concorso attivi
        $queryContents = ProgrammingActsModel::where('public_in_public_works', 1)
            ->skip(Input::get('skip'))
            ->take(Input::get('take'))
            ->get();
    ...
    private function generateCsv($resultContests = null, $resultSection = null): void
    {
        ...
        $sectionName = str_replace(',', '', $resultSection['name']);

        $fileName = 'Atti di Programmazione - ' . $sectionName. '.csv';
        header('Set-Cookie: fileDownload=true; path=/');
        ...
        foreach ($resultContests as $r) {
            ...
            $data = [
                ...
                // Tipo Atto
                !empty($r['act_type']) ? $types[$r['act_type']] : null,
                // Pubblica in "Opere Pubbliche"
                !empty($r['public_in_public_works']) ? 'Si' : 'No',
                // Descrizione
