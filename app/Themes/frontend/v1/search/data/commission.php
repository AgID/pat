...
<div class="wrapper-risultati">
    <?php
        foreach ($results['data'] as $item) :
            $link = siteUrl('page/245/details/' . $item['id'] . '/' . urlTitle($item['name']));
        ?>
    <article>
        <div class="box-risultato">
            <div>
                <a class="categoria"
                    href="<?php echo ($item['typology'] == 'commissione') ? siteUrl('page/245/commissioni') : siteUrl('page/244/gruppi-consiliari') ?>"><span
                        class="fas fa-university"></span> <?php echo escapeXss($item['typology']); ?></a>
                <h3>
                    <a
                        href="<?php echo $link ?>"><?php echo !empty($item['name']) ? escapeXss($item['name']) : 'N.D.' ?></a>
                </h3>

                <?php if (!empty($item['president'])) : ?>
                <p>
                    <span>Presidente:</span> <span
                        class="grigio"><?php echo escapeXss($item['president']['full_name']); ?></span>
                </p>
                <?php endif; ?>
            </div>
            <div>
