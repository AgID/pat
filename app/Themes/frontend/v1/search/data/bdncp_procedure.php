...
<?php if (!empty($results['data'])) : ?>
    <div class="wrapper-risultati">
        ...
        foreach ($results['data'] as $item) :
            $link = siteUrl('page/10/details/' . $item['id'] . '/' . urlTitle($item['cig'])); ?>
            <article>
                <div class="box-risultato">
                    <div>
                        <a class="categoria" href="<?php echo siteUrl('page/10/bandi-di-gara-e-contratti') ?>"><span
                                class="fas fa-university"></span>Bandi di gara e contratti</a>
                        <h3>
                            <a
                                href="<?php echo $link ?>"><?php echo !empty($item['object']) ? escapeXss($item['object']) : 'N.D.' ?></a>
                        </h3>

                        <?php if (!empty($item['cig'])) : ?>
                            <p>
                                <span>CIG:</span> <span class="grigio"><?php echo escapeXss($item['cig']); ?></span>
                            </p>
                        <?php endif; ?>
                    </div>
                    <div>
