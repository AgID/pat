...
<div class="wrapper-risultati">
    <?php
        foreach ($results['data'] as $item) :
            $link = siteUrl('page/114/details/' . $item['id'] . '/' . urlTitle($item['object'])); ?>
    <article>
        <div class="box-risultato">
            <div>
                <!-- Nome archivio -->
                <a class="categoria" href="<?php echo siteUrl('page/112/atti-delle-amministrazioni') ?>"><span
                        class="fas fa-university"></span> Atti delle amministrazioni</a>
                ...
                <!-- Procedura relativa all'atto -->
                <?php if (!empty($item['relative_contest_act'])) : ?>
                <p>
                    <span>Procedure relative:</span>
                    <?php foreach ($item['relative_contest_act'] as $contest){ ?>
                    <a
                        href="<?php echo siteUrl('page/110/details/' . $contest['id'] . '/' . urlTitle($contest['object'])); ?>">
                        <?php echo escapeXss($contest['object']); ?>
                    </a><br/>
                <?php } ?>
                </p>
                <?php endif; ?>
